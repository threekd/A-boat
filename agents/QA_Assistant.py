...
# 脚本位于 agents/ 子目录，运行时需手动把项目根目录加入 sys.path，
# 否则 `patches` 包无法导入。
...
def create_gradio_ui():
    """创建Gradio Web UI"""
    # 全局助手实例
    assistant_state: Dict[str, Optional[PDFLearningAssistant]] = {"assistant": None}
...
def _patch_gradio_client_bool_schema():
    """pydantic 2.11+ 的 model_json_schema() 会给 BaseModel 生成
    "additionalProperties": false，而 gradio_client 1.3.0 解析时会
    TypeError（"const" in True）。布尔 schema 直接当作 Any 处理。"""
    ...
    setattr(gcu, "_json_schema_to_python_type", _patched)
